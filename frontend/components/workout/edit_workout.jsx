...
class EditWorkout extends React.Component {
    ...
    componentDidMount() {
        this.props.fetchWorkout(this.props.workoutId).then((workout) => {

            let newPinsArray = workout.workout.coordinates.split('X').map(val => parseFloat(val)).slice(0, -1)
            let finalPinsArray = []
            for (let i=0; i< newPinsArray.length; i+=2){
                finalPinsArray.push({location: {lat: newPinsArray[i], lng: newPinsArray[i+1]}})
            }
            this.setState({title: workout.workout.title, 
                description: workout.workout.description,
                workout_type: workout.workout.workout_type,
                user_id: workout.workout.user_id,
                pins: finalPinsArray
